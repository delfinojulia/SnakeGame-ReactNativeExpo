...
import { COLS, FOOD_START, HEADER_HEIGHT, INCREMENT, PIXEL, SNAKE_START, SPEED, } from "../consts";
...
const Game = () => {
  ...
  const ROWS = Math.floor(
    (Height - top - insets?.bottom - HEADER_HEIGHT) / PIXEL
  );
  ...
  function testEatFood(snakeHead, foodLocation) {
    return (
      snakeHead.x === foodLocation.x && snakeHead.y === foodLocation.y
    );
  }
    ...
    <PanGestureHandler onGestureEvent={handleGesture}>
      <SafeAreaView style={styles.container}>
        <Header
          ...
          score={score}
          paused={isGamePaused}
          onPause={() => setIsGamePaused((prev) => !prev)}
